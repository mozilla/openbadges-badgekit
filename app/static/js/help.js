$(document).ready(function() {
  ...
  var faqItems = [
    ...
    },
    {
      ...
      answer: '<p>No! Don\'t worry if your design skills are not your strongpoint - the BadgeKit visual designer makes it incredibly easy to design a badge that uses attractive colors and visual elements, as well as scaling up and down nicely in different user environments. Alternatively, if you do have design skills on hand and want to use them in the badges you create, you can upload them into BadgeKit, rather than using the design tool.</p>'
    },
    {
    ...
    {
      question : 'Who can access badge applications in BadgeKit?',
      answer : '<p>The <span class="bold">Account Settings</span> section in BadgeKit allows you to manage user accounts for people you want to have access to BadgeKit. You can optionally use three levels of admin for your BadgeKit instance/ account: System, Issuer and Program.</p><p>To use BadgeKit, you need to define at least a System, which may simply be the name of your organization. You can organize all of your badges and applications within this one System if that is all you need. However, the additional admin levels of Issuer and Program allow you to organize your badging and configure user permissions.</p><p>For example, some reviewers in your badging system may only be able to review applications for a particular issuing organization (e.g. a school, library or other organization) or learning program (for example a festival or event). In the <span class="bold">Account Settings</span>, you can give users access to particular Issuer or Program badges.</p> '
    },
    {
      question : 'What happens when I review a badge application in BadgeKit?',
      answer : '<p>When someone reviews a badge application in BadgeKit, an issuer site can receive notification of the review using a webhook URL. Webhook URLs are configured within your BadgeKit API, and can receive notification of events.</p><p>Note that when an assessor submits an application review, indicating that the earner meets the requirements for the badge, this does not mean that the badge is issued to the earner straight away. When the application review is submitted in BadgeKit, the API webhook URL receives notification and the issuer can then contact the earner to inform them of the application review. If the earner then decides to accept the badge, the issuer site can use the API endpoints to award the badge.</p> '
    },
    {
      question : 'What is the difference between a badge and a badge instance?',
      answer : '<p>BadgeKit models awarded badges as badge instances. This distinguishes an instance of a badge awarded to a specific earner from the generic data for a badge. When a badge application is reviewed and the earner is awarded it, a badge instance is created for that particular badge award. In the technical implementation of Open Badges, an awarded badge is represented using an <span class="bold">assertion</span>.</p> '
    },
    {
    ...
    },
    {
      ...
      answer : '<p>When you create a badge, you can opt to choose types, tags and categories for it. These can be used within badge listings in the issuer site and to search for particular kinds of badge. Whether or not you need these types of categorization depends on your own badging programs.</p>'  
    }
  ];
